...
def check_Rows(room):
    ...
    total = 0
    #A flag to check if a piece of paper has been removed
    flag = True

    while flag == True:
        #Set the flag to false for this iteration as no paper has been removed yet
        flag = False
            ...
            for position in range(len(room[row])):
                ...
                if room[row][position] == '@':
                    ...
                    if (check_Adjacency(section)):
                        #Update this location to represent the paper being removed
                        room[row] = room[row][:position] + '.' + room[row][position + 1:]
                        #Set the flag to True as a piece of paper has been removed.
                        flag = True
