...
while len(circuits[0]) != len(positions):
        ...
        if distances[x][1] in circuits[circuit]:
            ...
            if distances[x][2] not in circuits[circuit]:
                ...
                #The other coordinate is in another circuit:
                    ...
                    #If this connection joins every position to one circuit:
                    if len(circuits[0]) == len(positions):
                        #Multiply together the x coordinates of the two positions
                        answer = distances[x][1][0] * distances[x][2][0]
    ...
    #Increments the counter to add the next positions to the circuits.
    x += 1
